...
local Library do
    ...
    Library = {
        ...
        SetFlags = setmetatable({}, {
            __newindex = function(t, k, v)
                rawset(t, k, v)
                if Library and Library.AutoSave then
                    pcall(function()
                        Library:AutoSave()
                    end)
                end
    ...
    Library.AutoSaveConfigPath = Library.Folders.Directory .. "/AutoSaveConfig.json"
    Library.AutoSaveThemePath = Library.Folders.Directory .. "/AutoSaveTheme.json"
    Library.AutoSaveDebounce = false

    Library.AutoSave = function(self)
        if Library.AutoSaveDebounce then return end
        Library.AutoSaveDebounce = true

        task.spawn(function()
            task.wait(0.5)
            Library.AutoSaveDebounce = false
            pcall(function()
                writefile(Library.AutoSaveConfigPath, Library:GetConfig())
            end)
        end)
    end

    Library.AutoSaveThemeNow = function(self)
        pcall(function()
            writefile(Library.AutoSaveThemePath, Library:GetTheme())
        end)
    end
    ...
    Library.Init = function(self)
        ...
        pcall(function()
            if isfile(Library.AutoSaveConfigPath) then
                AutoloadConfig = readfile(Library.AutoSaveConfigPath)
            end
        end)

        pcall(function()
            if isfile(Library.AutoSaveThemePath) then
                AutoloadTheme = readfile(Library.AutoSaveThemePath)
            end
        end)
        ...
        if AutoloadConfig ~= "" then
            ...
            if Success then
                Library:Notification({
                    Name = "Success",
                    Description = "Auto-loaded saved config",
                    Duration = 3,
                    Icon = "116339777575852",
        ...
        if AutoloadTheme ~= "" then
            ...
            if Success then
                Library:Notification({
                    Name = "Success",
                    Description = "Auto-loaded saved theme",
                    Duration = 3,
                    Icon = "116339777575852",
                    IconColor = Color3.fromRGB(52, 255, 164)
                })
        ...
        Library:Connect(game:GetService("Players").LocalPlayer.AncestryChanged, function()
            pcall(function()
                Library:AutoSave()
                Library:AutoSaveThemeNow()
            end)
        end)
    end
end 
